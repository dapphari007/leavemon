...
6. Initialize the database with all required data:
   ```
   npm run db:init
   ```
...
The system uses a consolidated migration process to initialize the database and set up all required data:
...
This command will:
1. Run all pending TypeORM migrations
2. Ensure database tables have the correct structure
3. Fix any data issues
4. Initialize system data (roles, pages, super admin)
5. Create default data (users, leave types, departments, positions, holidays)
...
- `npm run db:fresh` - Reset the database and start the server
