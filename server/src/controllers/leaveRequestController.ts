...
export const getManagerLeaveRequests = async (
  ...
  try {
    ...
    // Get leave requests with specific columns to avoid schema issues
    const leaveRequests = await leaveRequestRepository
      .createQueryBuilder("leaveRequest")
      .leftJoinAndSelect("leaveRequest.user", "user")
      .leftJoinAndSelect("leaveRequest.leaveType", "leaveType", "leaveType.id = leaveRequest.leaveTypeId")
      .select([
        "leaveRequest",
        "user.id", "user.firstName", "user.lastName", "user.email", "user.role",
        "leaveType.id", "leaveType.name", "leaveType.description", "leaveType.defaultDays", 
        "leaveType.isCarryForward", "leaveType.maxCarryForwardDays", "leaveType.isActive",
        "leaveType.applicableGender", "leaveType.isHalfDayAllowed", "leaveType.isPaidLeave",
        "leaveType.isPaid", "leaveType.requiresApproval", "leaveType.color"
      ])
      .where(query)
...
export const updateLeaveRequestStatus = async (
  ...
  try {
    ...
    // Define applicableWorkflow at a higher scope so it's available for all approval paths
    ...
    if (normalizedStatus === LeaveRequestStatus.APPROVED) {
      ...
      if (applicableWorkflow) {
        ...
        if (currentApproverLevel < highestRequiredLevel) {
          ...
          // Store the workflow details for UI display
          if (applicableWorkflow) {
        ...
        // Store the workflow details for UI display if not already present
        if (!metadata.workflowDetails && applicableWorkflow) {
          metadata.workflowDetails = {
            name: applicableWorkflow.name,
            approvalLevels: applicableWorkflow.approvalLevels
          };
    ...
    // For direct approvals without partial approval first
    else if (normalizedStatus === LeaveRequestStatus.APPROVED) {
      // Create metadata for direct approvals
      const metadata = leaveRequest.metadata || {};
      ...
      // If we have a workflow, store its details
      if (applicableWorkflow) {
        metadata.workflowDetails = {
          name: applicableWorkflow.name,
          approvalLevels: applicableWorkflow.approvalLevels
